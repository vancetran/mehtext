...
describe('textTransform', () => {
  describe('sarcastic', () => {
    test('Each letter is output in predictable alternating case when random is false', () => {
      expect(
        sarcastic(
          `I need healthcare because I have cancer and I'm dying`,
          false
        )
      ).toBe(`I nEeD HeAlThCaRe bEcAuSe I hAvE CaNcEr aNd I'm dYiNg`)
    })
  })
  ...
  describe('ssssnake', () => {
    test('Each word beginning with lowercase or uppercase S is prefixed with extra SSS characters', () => {
      expect(
        ssssnake(
          `Tunnel snakes rule, We're the tunnel snakes, That's us, And we rule! Rule!`
        )
      ).toBe(
        `Tunnel ssssnakes rule, We're the tunnel ssssnakes, That'ssss us, And we rule! Rule!`
      )
    })
  })
})
