...
export function sarcastic(input: string, random: boolean = false): string {
  const result = input
    .split('')
    .map((el: string, index: number): string => {
      ...
      return index % 2 ? el.toUpperCase() : el
    })
    .join('')
  return result
}
...
export function ssssnake(input: string): string {
  ...
  const arrResult = arr.map((el: string): string => {
    ...
    if (el.charAt(0) === 's' || el.charAt(0) === 'S') {
      if (el.charAt(0) === 's') return `sss${el}`
      return `SSS${el}`
    }
    if (el.charAt(lastIndex) === 's' || el.charAt(lastIndex) === 'S') {
      if (el.charAt(lastIndex) === 's') return `${el}sss`
      return `${el}SSS`
    }
    return el
  })
